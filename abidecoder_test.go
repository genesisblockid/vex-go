...
func TestABI_DecodeAction(t *testing.T) {
	...
	assert.Equal(t, "genesisblockid", gjson.GetBytes(json, "struct_1_field_1").String())
...
func TestABI_decodeFields(t *testing.T) {
	...
	assert.JSONEq(t, `{"F1":"18446744073709551615", "F2":"genesisblockid"}`, toJSON(t, json))
}
...
func TestABI_Read(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "Name", typeName: "name", data: Name("genesisblockid"), expectedValue: s(`"genesisblockid"`)},
		...
		{name: "extended_asset", typeName: "extended_asset", data: ExtendedAsset{Asset: Asset{Amount: 10, Symbol: EOSSymbol}, Contract: "genesisblockid"}, expectedValue: s("{\"quantity\":\"0.0010 EOS\",\"contract\":\"genesisblockid\"}")},
